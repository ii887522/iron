...
fn watch_a_reactive_object_with_initial_value_equal_to_0() {
  ...
  let value_squared = value.borrow_mut().watch(move |&value| value * value);
  ...
  let value_str = value.borrow_mut().watch(move |&value| format!("{value}"));
...
fn watch_a_reactive_object_with_initial_value_equal_to_a() {
  ...
  let repeated_str = string
    .borrow_mut()
    .watch(move |&value| format!("{value}{value}"));
  ...
  let number = string
    .borrow_mut()
    .watch(move |&value| value.parse::<i32>());
...
fn test_unwatch() {
  let value = reactive::Handle::from(0);
  let squared_value = value.borrow_mut().watch(move |&value| value * value);
